			...
			@else
				@foreach ($statuses as $status)
					<div class="media">
						...
						{{-- Auth::user()->id === $status->user->id, means i can reply to my own status --}}
		...
		<div class="col-lg-4 col-lg-offset-3">
			...
			@elseif (Auth::user()->isFriendWith($user))
				...
				<form action="{{ route('friends.delete', ['username' => $user->username]) }}" method="post">
					<input type="submit" value="Delete friend" class="btn btn-primary btn-sm">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
				</form>

			@elseif (Auth::user()->id !== $user->id)
				<a href="{{ route('friends.add', ['username' => $user->username]) }}" class="btn btn-primary btn-sm">Add as friend</a>
			@endif
